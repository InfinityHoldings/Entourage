...
public class UserController extends Controller {
	...
	public static Result signUp() {
		...
		Logger.debug("ENTOURAGE API: signUp() : incoming json: "
				+ json.toString() + "\n");
		try {
			...
			EntourageUser user = new EntourageUser(userName, email, city,
					state, password);
			Logger.debug("ENTOURAGE API: signUp(): New EntourageUser :",
					user.toString() + "\n");
			session.save(user);
			tx.commit();
	...
	public static Result getUserDetail() {
		...
		// Get the static instance of the Hibernate session from the
		// HibernateUtil class
		// based on new info from the tutorial, we may want to create/destroy
		// them as needed
		...
		Transaction tx = null;
		if (json == null) {
			Logger.debug("ENTOURAGE API: getUserDetail() : incoming json was null");
			return badRequest("Expecting Json data");
		}
		Logger.debug("ENTOURAGE API: getUserDetail() : incoming JSON : "
				+ json.toString());
		try {
			tx = session.beginTransaction();
			...
			@SuppressWarnings("unchecked")
			Iterator<EntourageUser> iterator = query.list().iterator();
			while (iterator.hasNext()) {
				EntourageUser user = iterator.next();
				String username = user.getUserName();
				String city = user.getCity();
				String state = user.getState();

				Logger.debug("ENTOURAGE API: getUserDetail(): EntouageUser : "
						+ user.toString());
	...
	public static Result updateUser() {
		...
		// is it using multiple sessions for the getidbyusername method call and
		// the udateuser call?
		...
		try {
			...
			// call session.save instead of the line below. take advantage of
			// the orm
			// refactor buildUpdateQuery method to call selective object setters
			// instead(i believe this is the strategy pattern)
	...
	static String buildUpdateQuery(JsonNode json) {
		// Check the json paths for the specific groups/arrays. build query
		// containing subset for each applicable group
		String sql = "";
